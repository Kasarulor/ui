 ...
 *
 *   copy  from  keyboardutils
 * */

public class KeyBoardUtils2_password {
...
//    public KeyBoardUtils2_password(Context context, TextView... targetTextView) {
//        this(context, targetTextView, KeyBoardType.values());
//    }

    private StringBuilder codeStringbBuilder = new StringBuilder();
    private TextView code;
    private Button sendcode;
    private View passwordUI;
    private boolean printToPassword = false;

    private onSendCodeCallBack<Button> onSendCodeCallBack;

    public void setOnSendCodeCallBack(com.kasarulor.ui.keyboard.onSendCodeCallBack<Button> onSendCodeCallBack) {
        this.onSendCodeCallBack = onSendCodeCallBack;
    }

    public KeyBoardUtils2_password(Context context) {
        this.context = context;
        this.keyBoardType = new KeyBoardType[]{KeyBoardType.NUMBER};


        mainView = LayoutInflater.from(context).inflate(R.layout.mainkeyboard2_number, null);
        code = mainView.findViewById(R.id.code);
        sendcode = mainView.findViewById(R.id.sendcode);
        sendcode.setOnClickListener(v -> {
           if(onSendCodeCallBack !=null){
               onSendCodeCallBack.sendCode(sendcode);
           }
        });
        passwordUI = mainView.findViewById(R.id.passui);
        printToPassword = false;
        passwordUI.setOnClickListener((v) -> {
            if (code.getText().toString().trim().equals("")) {
                printToPassword = false;
                Toast.makeText(context, "请先输入验证码", Toast.LENGTH_LONG).show();
            } else {
                printToPassword = true;
            }
        });
        code.setOnClickListener(v -> {
            printToPassword = false;
        });
        this.targetTextView = new TextView[6];
        this.targetTextView[0] = passwordUI.findViewById(R.id.i1);
        this.targetTextView[1] = passwordUI.findViewById(R.id.i2);
        this.targetTextView[2] = passwordUI.findViewById(R.id.i3);
        this.targetTextView[3] = passwordUI.findViewById(R.id.i4);
        this.targetTextView[4] = passwordUI.findViewById(R.id.i5);
        this.targetTextView[5] = passwordUI.findViewById(R.id.i6);
    ...
    private void initNumberView() {
        ...
        for (int number : numberSet) {
            ...
            numberText[index].setOnClickListener((v) -> {
                ...
                if (printToPassword) {
                    ...
                    codeStringbBuilder.append(numberText[finalIndex].getText().toString().trim());
                    printToPassword();
                }
            });
            index++;
        ...
        delete.setOnClickListener((v) -> {
            if (printToPassword) {
                ...
                if (codeStringbBuilder.toString().length() > 0) {
                    codeStringbBuilder.delete(codeStringbBuilder.toString().length() - 1, codeStringbBuilder.toString().length());
    ...
    private void printToPassword() {
        code.setText(codeStringbBuilder.toString());
...
//        return super.equals(obj);
    ...
    public void cache() {
        codeStringbBuilder.delete(0, codeStringbBuilder.toString().length());
        code.setText("");
        printToPassword = false;
